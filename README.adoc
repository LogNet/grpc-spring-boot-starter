...
----
Global interceptors can be ordered using Spring's `@Ordered` or `@Priority` annotations. Note that Grpc executes the given interceptor list for a call in reverse order.
So if you have interceptor A that relies on interceptor B to execute first you should define them like this:
