...
Please refer to the sections on <<Spring Security Integration>> for details on how to configure and supported authentication providers.
...
.Dependencies to implement authentiction scheme
[cols="a,a"]
|===
|Scheme |Dependencies
...
==== Default
Defining bean with type `GrpcSecurityConfigurerAdapter` is sufficient to secure you GRPC services and/or methods :
...
This default configuration secures methods/services annotated with `org.springframework.security.access.annotation.@Secured`  annotation. +
